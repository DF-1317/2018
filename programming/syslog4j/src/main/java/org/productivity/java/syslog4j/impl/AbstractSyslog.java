...
* @version $Id: AbstractSyslog.java,v 1.23 2010/02/11 05:00:28 cvs Exp $
*/
public abstract class AbstractSyslog implements SyslogIF {
	...
	protected void write(SyslogMessageProcessorIF messageProcessor, int level, String message) throws SyslogRuntimeException {
		String header = messageProcessor.createSyslogHeader(this.syslogConfig.getFacility(),level,this.syslogConfig.isSendLocalTimestamp(),this.syslogConfig.isSendLocalName());

		int messageLen = message.length();
		int headerLen = header.length();
		int availableLen = this.syslogConfig.getMaxMessageLength() - headerLen;
		...
		if (messageLen <= availableLen) {
			byte[] data = messageProcessor.createPacketData(h,m,0,messageLen);
